...
description: Monthly CEO scorecard for Andrés (Swapps) — one digest consolidating the month's key numbers across web traffic & leads (GA4/Search Console + Pipedrive CRM), paid campaigns (Google Ads), sales pipeline (Pipedrive + ClickUp Deals), finance (QuickBooks P&L, AR/AP, cash), and recurring revenue (swapps-app contracts), month-over-month where possible. Read-only. Posts one digest to the CEO Briefings channel.
---
...
> (if available), **pipedrive** (CRM leads/pipeline), **QuickBooks**, **swapps-app**, and
> **ClickUp** (to post).
...
2. **Leads (pipedrive MCP)** — `list_persons` / `list_deals` created in the month (and
   `list_activities` for first-touch): new leads count + sources, and how many became
   qualified / turned into deals. The ClickUp Leads channel is a secondary cross-check.
3. **Paid campaigns (Google Ads MCP, if connected)** — spend, conversions, cost/lead.
4. **Pipeline (pipedrive MCP, cross-checked vs ClickUp Deals `901405812778`)** — deals
   won/lost + value (`list_deals` by status, `get_deal` for detail), win rate, average
   quote turnaround vs the 4h target. Trust Pipedrive if the ClickUp mirror disagrees.
